...
[![build-ublue](https://github.com/howyay/test-ublue/actions/workflows/build.yml/badge.svg)](https://github.com/howyay/test-ublue/actions/workflows/build.yml)
...
> **Note**
> Everywhere in this repository, make sure to replace `howyay/test-ublue` with the details of your own repository. Unless you used [`create-ublue-image`](https://github.com/EinoHR/create-ublue-image), in which case the previous repo identifier should already be your repo's details.
...
Read [the README in the `scripts/` directory](https://github.com/howyay/test-ublue/blob/main/scripts/README.md) for more information.
...
- First rebase to the image unsigned, to get the proper signing keys and policies installed:
  ```
  sudo rpm-ostree rebase ostree-unverified-registry:ghcr.io/howyay/test-ublue:latest
  ```
...
- Then rebase to the signed image, like so:
  ```
  sudo rpm-ostree rebase ostree-image-signed:docker://ghcr.io/howyay/test-ublue:latest
  ```
...
```
sudo rpm-ostree rebase ostree-image-signed:docker://ghcr.io/howyay/test-ublue:20230403
```
